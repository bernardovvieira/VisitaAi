...
<x-section-card class="space-y-5 rounded-2xl border border-slate-200/80 bg-white/95 p-5 shadow-sm dark:border-slate-700/70 dark:bg-slate-900/50">
    <div class="flex items-center justify-between gap-3 border-b border-slate-200/80 pb-3 dark:border-slate-700/70">
        <h2 class="text-lg font-semibold text-slate-800 dark:text-slate-100">{{ __('Local visitado') }}</h2>
    ...
    <dl class="grid grid-cols-1 gap-x-6 gap-y-4 text-sm text-slate-700 dark:text-slate-300 sm:grid-cols-4">
        <div>
            <dt class="text-xs font-medium uppercase tracking-wide text-slate-500 dark:text-slate-400">{{ __('Código único do imóvel') }}</dt>
            <dd class="mt-1">
        ...
        <div>
            <dt class="text-xs font-medium uppercase tracking-wide text-slate-500 dark:text-slate-400">{{ __('Código da localidade') }}</dt>
            <dd class="mt-1">{{ $visita->local->loc_codigo ?? $na }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium uppercase tracking-wide text-slate-500 dark:text-slate-400">{{ __('Categoria da localidade') }}</dt>
            <dd class="mt-1">{{ $visita->local->loc_categoria ?? $na }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium uppercase tracking-wide text-slate-500 dark:text-slate-400">{{ __('Tipo') }}</dt>
            <dd class="mt-1">{{ $locTipoLabels[$visita->local->loc_tipo] ?? $na }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium uppercase tracking-wide text-slate-500 dark:text-slate-400">{{ __('Zona') }}</dt>
            <dd class="mt-1">{{ $locZonaLabels[$visita->local->loc_zona] ?? $na }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium uppercase tracking-wide text-slate-500 dark:text-slate-400">{{ __('Quarteirão') }}</dt>
            <dd class="mt-1">{{ $visita->local->loc_quarteirao ?? $na }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium uppercase tracking-wide text-slate-500 dark:text-slate-400">{{ __('Sequência') }}</dt>
            <dd class="mt-1">{{ $visita->local->loc_sequencia ?? $na }}</dd>
        </div>
        <div>
            <dt class="text-xs font-medium uppercase tracking-wide text-slate-500 dark:text-slate-400">{{ __('Lado') }}</dt>
            <dd class="mt-1">{{ $visita->local->loc_lado ?? $na }}</dd>
        </div>
        <div class="sm:col-span-4 border-t border-slate-200/80 pt-3 dark:border-slate-700/70">
            <dt class="text-xs font-medium uppercase tracking-wide text-slate-500 dark:text-slate-400">{{ __('Endereço completo') }}</dt>
            <dd class="mt-1 text-slate-900 dark:text-slate-100">
                ...
                <span class="text-sm text-slate-600 dark:text-slate-400">{{ __('Complemento') }}: {{ $visita->local->loc_complemento ?? $na }}</span>
            </dd>
        </div>
        <div class="sm:col-span-4">
            <dt class="text-xs font-medium uppercase tracking-wide text-slate-500 dark:text-slate-400">{{ __('Responsável pelo imóvel') }}</dt>
            <dd class="mt-1 text-slate-900 dark:text-slate-100">{{ $visita->local->loc_responsavel_nome ?? __('Não informado') }}</dd>
        </div>
    </dl>

    <dl class="grid grid-cols-1 gap-x-6 gap-y-4 text-xs text-slate-700 dark:text-slate-300 sm:grid-cols-2">
        <div>
    ...
    <div class="border-t border-slate-200/80 pt-4 dark:border-slate-700/70">
        <h3 class="mb-2 text-sm font-semibold text-slate-800 dark:text-slate-200">{{ __('Localização no mapa') }}</h3>
        <div id="map" class="h-72 rounded-xl border border-slate-200/90 shadow-sm dark:border-slate-700/80"></div>
    </div>
</x-section-card>
