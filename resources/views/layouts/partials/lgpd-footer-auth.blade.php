{{-- Rodapé LGPD compacto; não ocupa a largura total da área útil. --}}
@php
    $lgpd = config('visitaai_municipio.lgpd', []);
@endphp
<div class="mt-6 flex justify-center sm:mt-8">
    <details class="w-full max-w-xl rounded-lg border border-slate-200/80 bg-white/70 px-3 py-2 text-slate-700 shadow-sm dark:border-slate-600 dark:bg-slate-900/40 dark:text-slate-300">
        <summary class="cursor-pointer list-inside list-none text-center text-[11px] font-medium leading-snug text-slate-700 marker:hidden dark:text-slate-300 [&::-webkit-details-marker]:hidden">
