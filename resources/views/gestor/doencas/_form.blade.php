@php
    $isEdit = isset($doenca) && $doenca;
@endphp

<form method="POST" action="{{ $formAction }}" class="space-y-6" id="{{ $formId }}">
    @csrf
    @if($formMethod && strtoupper($formMethod) !== 'POST')
        @method($formMethod)
    @endif
    ...
    <div class="flex justify-end">
        <button type="submit" id="{{ $submitId }}" class="v-btn-compact v-btn-compact--blue">
            {{ $submitLabel }}
        </button>
    </div>
</form>
