...
<div class="space-y-6">
    ...
    @if(isset($visitasPaginated) && $visitasPaginated->total() > 0)
        <div class="mt-4 flex flex-wrap items-center justify-between gap-4 border-t border-gray-200 dark:border-gray-600 pt-4">
            <p class="text-sm text-gray-600 dark:text-gray-400">
                ...
                <span class="font-medium text-gray-800 dark:text-gray-200 ml-1">— Página {{ $visitasPaginated->currentPage() }} de {{ $visitasPaginated->lastPage() }}</span>
            </p>
            <nav class="flex items-center gap-2 flex-wrap" aria-label="Paginação">
                @if ($visitasPaginated->onFirstPage())
                ...
                @else
                    <a href="{{ $visitasPaginated->previousPageUrl() }}" class="px-3 py-2 rounded-lg bg-gray-200 dark:bg-gray-600 text-gray-800 dark:text-gray-200 text-sm font-medium hover:bg-gray-300 dark:hover:bg-gray-500 transition">Anterior</a>
                @endif
                @php
                    $current = $visitasPaginated->currentPage();
                    $last = $visitasPaginated->lastPage();
                    $start = max(1, $current - 2);
                    $end = min($last, $current + 2);
                @endphp
                @if ($start > 1)
                    <a href="{{ $visitasPaginated->url(1) }}" class="min-w-[2.25rem] px-3 py-2 rounded-lg bg-gray-200 dark:bg-gray-600 text-gray-800 dark:text-gray-200 text-sm font-medium text-center hover:bg-gray-300 dark:hover:bg-gray-500 transition">1</a>
                ...
                @for ($page = $start; $page <= $end; $page++)
                    ...
                    @else
                        <a href="{{ $visitasPaginated->url($page) }}" class="min-w-[2.25rem] px-3 py-2 rounded-lg bg-gray-200 dark:bg-gray-600 text-gray-800 dark:text-gray-200 text-sm font-medium text-center hover:bg-gray-300 dark:hover:bg-gray-500 transition">{{ $page }}</a>
                    @endif
                @endfor
                @if ($end < $last)
                    @if ($end < $last - 1) <span class="px-1 text-gray-400 dark:text-gray-500">…</span> @endif
                    <a href="{{ $visitasPaginated->url($last) }}" class="min-w-[2.25rem] px-3 py-2 rounded-lg bg-gray-200 dark:bg-gray-600 text-gray-800 dark:text-gray-200 text-sm font-medium text-center hover:bg-gray-300 dark:hover:bg-gray-500 transition">{{ $last }}</a>
                @endif
                @if ($visitasPaginated->hasMorePages())
                    <a href="{{ $visitasPaginated->nextPageUrl() }}" class="px-3 py-2 rounded-lg bg-gray-200 dark:bg-gray-600 text-gray-800 dark:text-gray-200 text-sm font-medium hover:bg-gray-300 dark:hover:bg-gray-500 transition">Próxima</a>
                @else
    ...
    @elseif(isset($visitasPaginated) && $visitasPaginated->total() == 0)
