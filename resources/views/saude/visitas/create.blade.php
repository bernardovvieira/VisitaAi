        ...
        <x-slot name="lead">
            ...
            <x-ui.disclosure variant="lead-mt">
                ...
                <p><strong>{{ __('Sem internet?') }}</strong>
                    {{ __('Use o botão «:btn» no final do formulário. A visita fica salva só no seu aparelho. Quando tiver conexão, abra «:menu» e clique em «:action» para enviar todas de uma vez.', [
            ...
            <fieldset class="space-y-3">
                ...
                <div>
                    ...
                    <select id="vis_atividade" name="vis_atividade" required
                            class="v-select mt-1">
                        <option value="7" selected>{{ $at7['codigo'] }} · {{ __($at7['nome']) }}</option>
                    </select>
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                        {{ __('O perfil ACS permite registrar a atividade LIRAa conforme Diretriz Nacional para Atuação Integrada dos ACE e ACS no Território (MS).') }}
                    </p>
                </div>
            </fieldset>
