...
<x-ui.disclosure variant="muted-card-simple" :open="false">
    ...
    <div class="space-y-3">
        ...
        <template x-for="(row, idx) in rows" :key="idx">
            <details x-bind:open="activeOcupante === idx" @toggle="activeOcupante = $event.target.open ? idx : null" class="rounded-lg border border-slate-200 bg-slate-50/50 p-4 dark:border-slate-600 dark:bg-slate-900/30">
                <summary :aria-expanded="activeOcupante === idx" class="cursor-pointer list-none font-semibold text-slate-700 marker:hidden dark:text-slate-200 [&::-webkit-details-marker]:hidden">
                    <div class="flex items-center justify-between gap-2">
                        <span class="inline-flex items-center gap-2 text-xs">
                            <svg :class="activeOcupante === idx ? 'ocupante-chevron rotate-90 w-3 h-3 text-slate-500 dark:text-slate-400' : 'ocupante-chevron w-3 h-3 text-slate-500 dark:text-slate-400'" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
