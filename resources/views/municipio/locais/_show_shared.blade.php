            ...
            <dl class="grid grid-cols-1 gap-x-4 gap-y-3 text-sm text-gray-700 dark:text-gray-300 sm:grid-cols-3">
                ...
                <div class="space-y-3">
                    <div>
                        ...
                        <dd class="mt-1">{{ $local->loc_codigo !== null && $local->loc_codigo !== '' ? '#'.$local->loc_codigo : $na }}</dd>
                    </div>
                    <div>
    ...
    @else
        @php
            ...
            $secoesSocio = [
                ...
                [
                    'titulo' => $t['proprietario'] ?? __('3. Proprietário (se aluguel / cedido)'),
                    'itens' => [
                ...
                [
                    'titulo' => $t['infraestrutura'] ?? __('6. Infraestrutura e serviços'),
                    'itens' => [
                ...
                [
                    'titulo' => $t['terreno'] ?? __('7. Terreno e tempo de residência'),
                    'itens' => [
                ...
                [
                    'titulo' => $t['historico'] ?? __('8. Histórico da posse'),
                    'itens' => [
                        ...
                        [__('Proprietário anterior'), $socio->lse_proprietario_anterior_nome],
                        [__('Doc. anterior'), $socio->lse_proprietario_anterior_doc],
                        [__('IPTU'), SE::opcao('sim_nao_curto_opcoes', $socio->lse_paga_iptu)],
                        [__('IPTU desde'), $socio->lse_iptu_desde],
                        [__('Situação legal'), $socio->lse_situacao_legal_obs],
                    ],
                ],
            ];
        @endphp
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const MAP_UNAVAILABLE = <?php echo json_encode(__('Mapa indisponível. Recarregue a página ou verifique sua conexão.')); ?>;
    const NO_COORDS = <?php echo json_encode(__('Coordenadas indisponíveis para este local.')); ?>;
    const MAP_FAIL = <?php echo json_encode(__('Não foi possível carregar o mapa.')); ?>;
    try {
