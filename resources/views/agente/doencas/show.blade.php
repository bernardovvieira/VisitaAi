...
@section('og_description', __('Detalhes da doença monitorada e sintomas para registro em campo.'))
  ...
  <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
    <div class="lg:col-span-3">
      <x-section-card :title="__('Registro')">
        <dl class="grid grid-cols-1 gap-x-6 gap-y-4 text-sm text-gray-700 dark:text-gray-300 sm:grid-cols-3">
          <div>
          ...
          <div>
            ...
            <dd class="mt-1 italic text-gray-900 dark:text-gray-100">{{ $doenca->created_at->format('d/m/Y H:i') }}</dd>
          </div>
          <div>
            <dt class="font-medium">{{ __('Atualizado em') }}</dt>
            <dd class="mt-1 italic text-gray-900 dark:text-gray-100">{{ $doenca->updated_at->format('d/m/Y H:i') }}</dd>
          </div>
        </dl>
      </x-section-card>
    </div>
