<!-- resources/views/agente/locais/show.blade.php -->
@extends('layouts.app')
...
<div class="container mx-auto p-6 max-w-4xl space-y-6">
    ...
    <section class="p-4 bg-white dark:bg-gray-700 rounded-lg shadow">
        ...
        <p class="mt-2 text-gray-600 dark:text-gray-400">
            Abaixo estão as informações completas do local cadastrado.
        </p>
    </section>
    ...
    <section class="bg-white dark:bg-gray-700 rounded-lg shadow p-6">
        <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 text-sm text-gray-700 dark:text-gray-300">
            ...
            <div>
                ...
                <dd class="mt-1 text-gray-900 dark:text-gray-100">{{ $local->loc_cep }}</dd>
            </div>
            <div class="sm:col-span-2">
                <dt class="font-medium">Endereço Completo</dt>
                <dd class="mt-1 text-gray-900 dark:text-gray-100">
                    {{ $local->loc_endereco }}, {{ $local->loc_numero }} - {{ $local->loc_bairro }}, {{ $local->loc_cidade }}/{{ $local->loc_estado }} - {{ $local->loc_pais }}
                </dd>
            </div>
            <div>
    ...
    <section class="bg-white dark:bg-gray-700 rounded-lg shadow p-4">
        <h3 class="text-md font-semibold text-gray-800 dark:text-gray-200 mb-2">Localização no Mapa</h3>
        <div id="map" class="h-72 rounded border border-gray-300"></div>
    </section>
</div>
