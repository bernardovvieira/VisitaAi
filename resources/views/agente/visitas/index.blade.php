...
<div class="max-w-7xl mx-auto space-y-6"
    ...
    <section class="p-4 bg-white dark:bg-gray-700 rounded-lg shadow">
        ...
        {{-- Offline: só index e create; editar/remover apenas rascunhos salvos no dispositivo --}}
        <section x-show="!online" x-cloak class="p-4 bg-amber-50 dark:bg-amber-900/20 rounded-lg shadow border border-amber-200 dark:border-amber-800 mt-6">
            ...
            <div id="visita-offline-drafts-list" class="mt-3 space-y-2"></div>
        </section>
    </section>
</div>
<script>
(function() {
    var createUrl = @json(route('agente.visitas.create'));
    var listEl = document.getElementById('visita-offline-drafts-list');
    if (!listEl) return;
                ...
                btn.addEventListener('click', function() {
                    ...
                    window.VisitaOfflineDeleteDraft(btn.getAttribute('data-draft-id')).then(function() {
                        ...
                        if (typeof window.VisitaOfflineGetDrafts === 'function') {
                            window.VisitaOfflineGetDrafts('agente').then(renderDrafts);
                        }
                    });
                });
            }
    ...
    function updateDraftsWhenOffline() {
        ...
        window.VisitaOfflineGetDrafts('agente').then(renderDrafts).catch(function() { listEl.innerHTML = '<p class="text-sm text-gray-500">Erro ao carregar rascunhos.</p>'; });
    }
